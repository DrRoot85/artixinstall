...
mkfs.vfat -F32 -n "EFI" /dev/nvme0n1
mkfs.btrfs -L ROOT /dev/nvme0n2
...
mount -o noatime,nodiratime,compress=zstd,commit=120,space_cache,ssd,discard=async,autodefrag,subvol=@artix /dev/nvme0n2 /mnt
mkdir -p /mnt/{boot,home,swap,btrfs,srv,tmp,.snapshots,var/{log,tmp,abs,cache/pacman/pkg}}
mount -o noatime,nodiratime,compress=zstd,commit=120,space_cache,ssd,discard=async,autodefrag,subvol=@home /dev/sda1 /mnt/home
mount -o noatime,nodiratime,compress=zstd,commit=120,space_cache,ssd,discard=async,autodefrag,subvol=@srv /dev/nvme0n2 /mnt/srv
mount -o noatime,nodiratime,compress=zstd,commit=120,space_cache,ssd,discard=async,autodefrag,subvol=@log /dev/nvme0n2 /mnt/var/log
mount -o noatime,nodiratime,compress=zstd,commit=120,space_cache,ssd,discard=async,autodefrag,subvol=@tmp /dev/nvme0n2 /mnt/var/tmp
mount -o noatime,nodiratime,compress=zstd,commit=120,space_cache,ssd,discard=async,autodefrag,subvol=@abs /dev/nvme0n2 /mnt/var/abs
mount -o noatime,nodiratime,compress=zstd,commit=120,space_cache,ssd,discard=async,autodefrag,subvol=@pkg /dev/nvme0n2 /mnt/var/cache/pacman/pkg
mount -o noatime,nodiratime,compress=zstd,commit=120,space_cache,ssd,discard=async,autodefrag,subvol=@snapshots /dev/nvme0n2 /mnt/.snapshots

mount -o noatime,nodiratime,compress=zstd,commit=120,space_cache,ssd,discard=async,autodefrag,subvolid=5 /dev/nvme0n2 /mnt/btrfs
mount -o compress=no,space_cache,ssd,discard=async,subvol=@swap /dev/nvme0n2 /mnt/swap
mount -t tmpfs -o rw,nosuid,nodev,exec,auto,nouser,async,noatime,mode=1777,size=10G /dev/nvme0n2 /mnt/tmp
...
basestrap /mnt base base-devel dinit elogind-dinit linux-firmware btrfs-progs zstd \
    neovim zsh libvirt qemu docker-dinit git man chrony-runit cronie-runit vim
...
# AMD GPU drivers
basestrap /mnt  mesa libva-mesa-driver mesa-vdpau xf86-video-intel
# Nvidia GPU drivers
...
menuentry "Artix Linux" {
    ...
    options  "root=PARTUUID=$(blkid -s PARTUUID -o value /dev/sda2) rootflags=subvol=@artix rw quiet console=tty2 nmi_watchdog=0 add_efi_memmap initrd=\intel-ucode.img"
...
sudo pacman -S snapper networkmanager-dinit network-manager-applet openssh-dinit \
    zsh-autosuggestions zsh-history-substring-search zsh-syntax-highlighting \
    libimobiledevice usbutils xorg xorg-xinit pipewire-dinit acpid-dinit backlight-dinit bluez-dinit pipewire-alsa \
    pipewire-pulse-dinit pipewire-jack pasystray pavucontrol  fuse-dinit nvidia-utils-dinit tlp-dinit turnstile-dinit wireplumber-dinit \
...
sudo ln -s ../{NetworkManager,sshd,chrony,cronie, } /etc/dinit/boot.d/
